...
Orchestrator service là một microservice đặc biệt, đóng vai trò là "nhạc trưởng" trong các giao dịch phân tán phức tạp (Saga). Dịch vụ này không chịu trách nhiệm xử lý nghiệp vụ đơn lẻ mà tập trung vào việc điều phối chuỗi gọi internal API đồng bộ để đảm bảo tính nhất quán giữa nhiều microservice.

\textbf{1. POST /api/order/place}

API này là điểm tiếp nhận yêu cầu Đặt hàng từ Frontend. Nó kích hoạt luồng Saga Orchestration và đồng thời hoạt động như một command gateway ghi nhận lệnh nghiệp vụ.

\begin{itemize}
    \item Chức năng: Khởi tạo luồng giao dịch đặt hàng (order placement saga).
    \item Đầu vào: Thông tin đơn hàng (item, quantity, shippingAddress, paymentMethod, \ldots).
    \item Xử lý (orchestrator logic):
    \begin{enumerate}
        \item Validate dữ liệu đầu vào.
        \item Ghi log $\text{SagaStarted}$ với trạng thái $\text{Pending}$ vào $\text{Saga Log}$.
        \item Thực hiện chuỗi gọi internal API đồng bộ theo thứ tự (saga steps): 
        \begin{itemize}
            \item Bước 1 (Giữ hàng): Gọi $\text{Product Service}$ ($\text{POST}$ $\text{/api/internal/product/reserve}$) để giữ tồn kho.
            \item Bước 2 (Thanh toán \& Tạo đơn): Nếu Bước 1 thành công, gọi $\text{Order Service}$ ($\text{POST}$ $\text{/api/internal/order/process-payment}$) để xử lý thanh toán và tạo đơn tạm thời.
            \item Kết thúc Saga: Nếu tất cả các bước đồng bộ thành công, orchestrator sẽ ghi sự kiện $\text{OrderPlacementCompleted}$ (hoặc tương tự) vào Event Store/Kafka.
            \item Hoàn tác (Compensation): Nếu bất kỳ bước nào thất bại (nhận mã HTTP $4xx$ hoặc $5xx$), orchestrator sẽ ngay lập tức kích hoạt chuỗi hoàn tác bằng cách gọi các $\text{Compensate API}$ tương ứng (ví dụ: $\text{/api/internal/product/compensate}$).
        \end{itemize}
    \end{enumerate}
    \item Đầu ra: $\text{BaseResponse}$ đồng bộ xác nhận đã kích hoạt Saga thành công (chứa $orderId$ và trạng thái $\text{PROCESSING}$), hoặc lỗi tức thời nếu Saga thất bại ngay từ bước đầu (ví dụ: validation error).
\end{itemize}
