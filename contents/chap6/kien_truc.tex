...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/chap6/ArchitectureDiagram_v2.png}
    \vspace{0.5cm}
...
Ngoài ra, về mặt cấu trúc nội bộ, mỗi Microservice được nhỏm tổ chức theo mô hình phân tầng cơ bản. 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{images/chap6/JavaBackend.png}
    \vspace{0.5cm} % Khoảng cách giữa 2 hình
    
    \includegraphics[width=0.85\textwidth]{images/chap6/PythonBackend.png}
    \vspace{0.5cm}
    \caption{Cấu trúc nội bộ Microservice của các dịch vụ Java (trên) và Python (dưới)}
    \label{fig:internal_service_architecture}
\end{figure}
\vspace{-0.5cm}
Sơ đồ \ref{fig:internal_service_architecture} mô tả cấu trúc nội bộ của các dịch vụ trong hệ thống, bao gồm dịch vụ nghiệp vụ nền tảng Java (phía trên) và dịch vụ AI Agent sử dụng ngôn ngữ Python (phía dưới). Cả hai loại dịch vụ đều áp dụng kiến trúc CQRS, tách biệt hoàn toàn luồng thay đổi dữ liệu và luồng truy vấn để tối ưu hóa hiệu năng cũng như khả năng mở rộng.

Tùy vào đặc thù chuyên môn, các dịch vụ có sự khác biệt rõ rệt về công nghệ điều phối và lưu trữ dữ liệu:
\begin{itemize}
    \item Dịch vụ Nghiệp vụ (Java): Sử dụng Vert.x Event Bus làm bộ điều phối nội bộ để chuyển giao các yêu cầu một cách bất đồng bộ. Luồng ghi dùng JPA Repository để quản lý các thực thể phức tạp và đảm bảo tính nhất quán dữ liệu trong cơ sở dữ liệu PostgreSQL. Ngược lại, luồng đọc sử dụng một kho lưu trữ riêng với Spring JdbcTemplate để thực hiện các câu lệnh SQL thuần, giúp loại bỏ độ trễ của các bộ thư viện ánh xạ thực thể và tăng tốc độ phản hồi.

    \item Dịch vụ AI Agent (Python): Tận dụng cơ chế Asyncio của FastAPI để xử lý đồng thời các tác vụ tính toán nặng. Đặc biệt, dịch vụ này vận hành đồng thời trên hai hệ thống lưu trữ chuyên biệt: cơ sở dữ liệu vector (Milvus) để xử lý tìm kiếm tương đồng và cơ sở dữ liệu đồ thị (LangGraph) để quản lý các mối liên kết tri thức. Tại tầng truy cập dữ liệu, dịch vụ sử dụng các bộ thư viện tương ứng như PyMilvus để tương tác song song. Luồng ghi đảm nhận việc trích xuất và đồng bộ thông tin vào cả hai kho lưu trữ, trong khi luồng đọc thực hiện truy vấn kết hợp để cung cấp phản hồi chính xác và giàu ngữ cảnh.
\end{itemize}

Điểm chung của cả hai là việc sử dụng Kafka Producer ở luồng ghi để phát đi các sự kiện nghiệp vụ. Đây là mắt xích quan trọng để kích hoạt các quy trình liên dịch vụ như SAGA, đảm bảo tính toàn vẹn dữ liệu cho toàn bộ hệ thống phân tán.
