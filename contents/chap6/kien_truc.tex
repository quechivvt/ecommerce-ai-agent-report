...
Ngoài ra, về mặt cấu trúc nội bộ, mỗi Microservice được nhỏm tổ chức theo mô hình phân tầng cơ bản (Controller → Service → Repository). 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.48\textwidth]{images/chap6/JavaBackend.png} 
    \quad
    \includegraphics[width=0.48\textwidth]{images/chap6/PythonBackend.png} 
    \vspace{0.5cm}
    \caption{Cấu trúc Nội bộ Microservice của các service Java (trái) và Python(phải)}
    \label{fig:internal_service_architecture}
\end{figure}

Sơ đồ \ref{fig:internal_service_architecture} mô tả cấu trúc nội bộ của từng Microservice. Ở mức này, dịch vụ tuân theo mô hình phân tầng gồm controller → service → repository để tách biệt rõ ràng chức năng. Tầng service được chia thành hai luồng xử lý: read flow và write flow.

Read flow truy vấn dữ liệu thông qua repository theo mô hình truyền thống. Ngược lại, write flow không thực hiện cập nhật trực tiếp mà ghi các thay đổi dưới dạng event vào event store (Kafka). Kafka đóng vai trò event store trong kiến trúc event sourcing, nơi toàn bộ sự kiện thay đổi trạng thái được lưu trữ tuần tự và có thể được sử dụng để xây dựng lại projection hoặc kích hoạt các xử lý bất đồng bộ thông qua các consumer.

Đối với các service nghiệp vụ thương mại điện tử và orchestrator service, hệ thống còn sử dụng vert.x event bus để hỗ trợ giao tiếp bất đồng bộ nội bộ service, giúp dịch vụ xử lý các tác vụ bên trong theo kiểu non-blocking, nâng cao hiệu năng và khả năng mở rộng. Trong khi đó, AI service không dùng vert.x event bus vì được xây dựng bằng Python và sử dụng FastAPI/asyncio cho mô hình bất đồng bộ, vốn đáp ứng tốt nhu cầu xử lý I/O phức tạp của dịch vụ AI.
